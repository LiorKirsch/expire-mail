var Gmail =  function() {
 ...
 api.dom.check_item_exists = function(unique_identifier, jqueryItem) {
	...
	if(previousItem.hasClass('customdec')) {
		...
		itemExists = api.dom.check_item_exists(unique_identifier, previousItem);
		}
	}
	return itemExists;
}
 ...
 api.dom.insert_before_reply_button_everytime = function(jqueryObject,unique_identifier) {
	$('body div.nH').contents().on('mouseover','.nH[role="main"]',function(){
		var reply_buttons = api.dom.get_reply_button();
		$(reply_buttons).each(function(loopindex,loopitem){
			if(! api.dom.check_item_exists(unique_identifier, $(this) ) ){
 ...
 api.dom.get_reply_button = function() {
